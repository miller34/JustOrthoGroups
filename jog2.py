...
def getAllSubSeq(input):
	...
	for x in range(len(sequence)-dirOverlap+1):
		allSub.append(sequence[x:x+dirOverlap])
	return cl.Counter(allSub)
...
def readInputFiles(args):
	...
	for inputFile in allInputFiles:
		...
		if sequence != "":
			...
			mySequences.append((sequence, dirOverlap))
			orthoGroup +=1
		results +=(pool.map(getAllSubSeq,mySequences,chunksize=1))
		input.close()
	return results
...
def getGroupings(numToHeader,results,threads):
	...
	for x in range(numInstances):
		if not (x+1) in groupings:
			groupings[x+1] = dict()
		for y in range(x,numInstances):
			if (y+1) in groupings:
				if (x+1) in groupings[y+1]:
					continue
			else:
				groupings[y+1] = dict()
			addThis = len(list((results[x] & results[y])))
			groupings[x+1][y+1] = addThis
			groupings[y+1][x+1] = addThis
	return groupings
